...
// useSyncExternalStore로 React와 연결. 데이터 유실 방지용 JSON 내보내기/가져오기 제공.
...
export function getDeal(id) {
  return state.deals.find((d) => d.id === id) || null;
}

/* ===== 머니테스트 ===== */
export function addMoneyTest(mt) {
  const now = new Date().toISOString();
  const rec = { id: uid(), createdAt: now, updatedAt: now, ...mt };
...
export function updateMoneyTest(id, patch) {
  setState((s) => ({
    ...s,
    moneyTests: s.moneyTests.map((m) => (m.id === id ? { ...m, ...patch, updatedAt: new Date().toISOString() } : m)),
  }));
}
export function removeMoneyTest(id) {
  setState((s) => ({
    ...s,
    moneyTests: s.moneyTests.filter((m) => m.id !== id),
    deals: s.deals.map((d) => (d.moneyTestId === id ? { ...d, moneyTestId: null } : d)),
  }));
}
export function getMoneyTest(id) {
  return state.moneyTests.find((m) => m.id === id) || null;
}
...
export function getWeekly(weekOf) {
  return state.weeklyReviews.find((w) => w.weekOf === weekOf) || null;
}
export function upsertWeekly(weekOf, patch) {
  setState((s) => {
    const exists = s.weeklyReviews.find((w) => w.weekOf === weekOf);
    if (exists) {
      return { ...s, weeklyReviews: s.weeklyReviews.map((w) => (w.weekOf === weekOf ? { ...w, ...patch, updatedAt: new Date().toISOString() } : w)) };
    }
...
export function importJSON(text) {
  ...
  if (!parsed || typeof parsed !== "object" || !("deals" in parsed)) throw new Error("성장원장 백업 파일 형식이 아닙니다.");
  state = { ...fresh(), ...parsed };
  emit();
}
