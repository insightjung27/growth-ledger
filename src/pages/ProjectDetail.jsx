...
import {
  ...
  addTask, updateTask, removeTask, updateStakeholder,
} from "../lib/store.js";
...
export default function ProjectDetail() {
  ...
  const pr = p ? projectProgress(p, tasks, handoffs) : null;
  const myTasks = useMemo(() => (p ? tasks.filter((t) => t.projectId === id) : []), [tasks, id, p]);
  const myHandoffs = useMemo(() => (p ? handoffs.filter((h) => (p.handoffIds || []).includes(h.id)) : []), [handoffs, p, id]);
  ...
  function toggleTaskDone(t) { updateTask(t.id, { status: t.status === "done" ? "todo" : "done" }); }
  function addProjTask() { if (!newTask.trim()) return; addTask({ title: newTask.trim(), projectId: id, status: "todo" }); setNewTask(""); }
  function addMs() { if (!newMs.trim()) return; addMilestone(id, { name: newMs.trim() }); setNewMs(""); }
  function del() { if (confirm("이 프로젝트를 삭제할까요? 하위 태스크는 Inbox로 복귀하고, 위임과제/이해관계자는 유지됩니다.")) { removeProject(id); nav("/projects"); } }
    ...
    <div>
      ...
      <div className="panel panel-pad section">
        ...
        <div className="between" style={{ alignItems: "center" }}>
          <span className="tiny muted">진척(태스크·위임·마일스톤 완료율)</span>
      ...
      {/* 직접수행 태스크 */}
      <div className="section">
        <div className="section-title">내가 할 일 (직접 수행)</div>
        <div className="panel panel-pad">
          {myTasks.length === 0 && <div className="muted small" style={{ marginBottom: 10 }}>이 프로젝트의 직접수행 태스크가 없습니다.</div>}
          <div className="stack">
            {myTasks.map((t) => (
          ...
          <div className="gap-wrap" style={{ marginTop: 10 }}>
            <input className="input" style={{ flex: 1, minWidth: 0 }} value={newTask} placeholder="내가 직접 할 일" onKeyDown={(e) => e.key === "Enter" && addProjTask()} onChange={(e) => setNewTask(e.target.value)} />
            <button className="btn btn-sm btn-primary" onClick={addProjTask} disabled={!newTask.trim()}>추가</button>
          </div>
        </div>
      </div>
      ...
      {/* 위임과제 연결 */}
      <div className="section">
        <div className="section-title">위임과제 (남에게 맡긴 일)</div>
        <div className="panel panel-pad">
          {myHandoffs.length ? (
